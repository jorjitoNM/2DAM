package com.example.apptareas.utilities

object Constantes {
    const val MESSAGE_RECOVERED: String = "Message recovered"
    const val UNDO: String = "Undo delete"
    const val LOADING: String = "We are processing your operation"
    const val EVENT_ID: String = "eventId"
    const val EVENT_UPDATED: String = "Event updated successfully"
    const val ERROR__GETTING_EVENT: String = "Could not get the specified event"
    const val LOGIN_ERROR = "Login failed, please check the fields and try again"
    const val GET_EVENTS_FAILED = "An error occurred when getting your events, please, try again"
    const val IMAGE_PROVIDER = "https://thispersondoesnotexist.com/"
    const val ERROR_UPDATING_EVENT = "Could not update the event, try again"
    const val INVALID_USERNAME = "Username must contain an a"
    const val PETITION_RETURNED_LOADING = "Ha devuelto loading y volvemos a llamar a la funcion"
    const val EVENT_DELETED = "Evento eliminado con exito"
    const val ERROR_DELETING_EVENT = "No se ha podido eliminar el evento"
    const val USER_ID = "user_id"
}